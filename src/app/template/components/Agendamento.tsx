...
export default function Agendamento() {
  ...
  return (
    <section id="agendar" style={{ background: "var(--color-dark-main)", padding: "96px 40px", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: -200, right: -200, width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(ellipse, rgba(184,150,62,.1) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ maxWidth: 1160, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start", position: "relative", zIndex: 1 }} className="cta-grid">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 20, background: "rgba(184,150,62,.15)", border: ".5px solid rgba(184,150,62,.3)", borderRadius: 9999, padding: "5px 14px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 14 }}>
            {["Atendimento rápido pelo WhatsApp", "Dados protegidos pela LGPD", "Avaliação personalizada e sem compromisso"].map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 20, height: 20, borderRadius: "50%", background: "rgba(184,150,62,.15)", border: ".5px solid rgba(184,150,62,.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column" as const, gap: 0 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }} className="form-row-grid">
              <div>
                <label style={{ display: "block", fontSize: 11, color: "rgba(255,255,255,.38)", marginBottom: 6 }}>Nome completo</label>
                <input type="text" required value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Seu nome" style={inputStyle}
                  onFocus={(e) => (e.target.style.borderColor = "rgba(184,150,62,.5)")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,.12)")} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 11, color: "rgba(255,255,255,.38)", marginBottom: 6 }}>WhatsApp</label>
                <input type="tel" required inputMode="tel" value={form.tel} onChange={(e) => setForm({ ...form, tel: e.target.value })} placeholder="(41) 9 9999-9999" style={inputStyle}
                  onFocus={(e) => (e.target.style.borderColor = "rgba(184,150,62,.5)")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,.12)")} />
              </div>
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 11, color: "rgba(255,255,255,.38)", marginBottom: 6 }}>Procedimento de interesse</label>
              <select value={form.proc} onChange={(e) => setForm({ ...form, proc: e.target.value })} style={{ ...inputStyle, color: form.proc ? "rgba(255,255,255,.85)" : "rgba(255,255,255,.45)", cursor: "pointer" }}
                ...
                {["Botox / Toxina Botulínica", "Preenchimento Labial", "Laser Dermatológico", "Rejuvenescimento Facial", "Bioestimuladores", "Dermatologia Clínica", "Outro"].map(o => <option key={o} style={{ color: "#111" }}>{o}</option>)}
              </select>
            </div>
            <button type="submit" style={{ width: "100%", background: "linear-gradient(135deg, var(--color-gold) 0%, var(--color-gold-dark) 100%)", color: "#fff", borderRadius: 9999, padding: 15, fontSize: 14, fontWeight: 600, marginTop: 8, cursor: "pointer", border: "none", fontFamily: "inherit", transition: "transform .2s, box-shadow .2s, opacity .2s" }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(184,150,62,.35)"; e.currentTarget.style.opacity = ".95"; }}
      ...
      <style>{`
        @media (max-width: 960px) { .cta-grid { grid-template-columns: 1fr !important; gap: 48px !important; } .form-row-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </section>
  );
}
