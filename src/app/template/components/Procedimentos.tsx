...
import { ArrowRight, Syringe, Sparkles, Sun, Droplets, Zap, Shield } from "lucide-react";
...
const procedimentos = [
  ...
  { icon: Droplets, nome: "Preenchimento Labial", desc: "Volume natural com ácido hialurônico. Contorno definido, hidratação profunda e resultado imediato." },
  { icon: Sun, nome: "Rejuvenescimento Facial", desc: "Protocolos combinados para renovação da pele, luminosidade e redução de manchas com tecnologia de ponta." },
  { icon: Zap, nome: "Laser Dermatológico", badge: "tecnologia", desc: "Tratamentos a laser para acne, cicatrizes, manchas e rejuvenescimento com precisão clínica." },
  { icon: Shield, nome: "Dermatologia Clínica", desc: "Diagnóstico e tratamento de condições dermatológicas com abordagem personalizada e segura." },
  { icon: Syringe, nome: "Bioestimuladores", desc: "Estimulação da produção de colágeno para firmeza, elasticidade e rejuvenescimento duradouro." },
];
  ...
  return (
    <section id="procedimentos" style={{ background: "var(--color-bg)", padding: "96px 40px" }}>
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: i * 0.08 }}
                ...
                <Link href="#agendar" className="proc-card-link" style={{ display: "block", height: "100%", padding: 24, background: "var(--color-surface)", border: ".5px solid var(--color-border-subtle)", borderRadius: 20, textDecoration: "none", color: "inherit", transition: "transform .25s, box-shadow .25s, border-color .2s" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    {proc.badge && <span style={{ fontSize: 9, background: "var(--color-gold-light)", color: "var(--color-gold-dark)", padding: "2px 8px", borderRadius: 9999, fontWeight: 500 }}>{proc.badge}</span>}
                  </div>
                  <p style={{ fontSize: 13, color: "var(--color-muted)", lineHeight: 1.6, marginBottom: 16 }}>{proc.desc}</p>
                  <div className="proc-link" style={{ fontSize: 12, color: "var(--color-gold)", fontWeight: 500, display: "flex", alignItems: "center", gap: 4 }}>
      ...
      <style>{`
        ...
        .proc-card-link:hover .proc-icon { background: rgba(184,150,62,.18); }
      `}</style>
    </section>
  );
}
