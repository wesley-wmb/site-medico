...
export default function Hero() {
  return (
    <section id="inicio" style={{ background: "radial-gradient(ellipse 70% 60% at 10% 65%, rgba(124,92,69,.08) 0%, transparent 65%), radial-gradient(ellipse 50% 50% at 88% 18%, rgba(124,92,69,.06) 0%, transparent 55%), var(--color-bg)", padding: "0 40px" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "88px 0 96px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }} className="hero-grid">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} style={{ display: "flex", flexDirection: "column" }} className="hero-content-col">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} style={{ display: "inline-flex", alignItems: "center", gap: 8, alignSelf: "flex-start", background: "var(--color-gold-light)", border: ".5px solid rgba(124,92,69,.3)", borderRadius: 9999, padding: "6px 14px", fontSize: 11, fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "var(--color-gold-dark)", marginBottom: 24 }}>
            ...
            Estética Avançada · São José dos Pinhais, PR
          </motion.div>

          <h1 className="heading-display" style={{ fontSize: "clamp(48px, 5.5vw, 82px)", marginBottom: 24 }}>
            Sua melhor<br />versão <em>começa</em><br />aqui.
          </h1>

          <p style={{ fontSize: 15, color: "var(--color-muted)", lineHeight: 1.75, maxWidth: 420, marginBottom: 36, fontWeight: 300 }}>
            Harmonização Facial e Corporal, Emagrecimento e Rejuvenescimento com tecnologia de ponta e atendimento personalizado.
          </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap" as const }}>
            {["Harmonização Facial", "Tricologia Avançada", "Resultados reais"].map((item, i) => (
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, delay: 0.1, ease: [0.16, 1, 0.3, 1] }} style={{ position: "relative", animation: "float 6s ease-in-out infinite" }} className="hero-img-col">
          <div style={{ width: "100%", aspectRatio: "4/5", borderRadius: 28, background: "linear-gradient(160deg, #e8e0d8 0%, #d5c8bc 100%)", position: "relative", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.1), 0 4px 16px rgba(0,0,0,.06)" }}>
            <img src="/doctor_portrait.jpg" alt="Dra. Marlena Benna" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
            <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at 40% 25%, rgba(124,92,69,.1) 0%, transparent 60%)", pointerEvents: "none" }} />
          </div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.8 }} style={{ position: "absolute", bottom: 32, left: -32, background: "rgba(255,255,255,.85)", backdropFilter: "blur(20px) saturate(160%)", WebkitBackdropFilter: "blur(20px) saturate(160%)", border: ".5px solid rgba(255,255,255,.9)", borderRadius: 20, padding: "14px 18px", boxShadow: "0 12px 40px rgba(0,0,0,.12), 0 2px 8px rgba(0,0,0,.06)", minWidth: 200 }}>
            ...
            <div className="font-display" style={{ fontSize: 30, fontWeight: 400, color: "var(--color-text-main)", letterSpacing: "-0.03em", lineHeight: 1, marginBottom: 2 }}>5.0</div>
            <div style={{ fontSize: 11, color: "var(--color-subtle)" }}>Avaliações no Google</div>
          </motion.div>
        </motion.div>
      </div>

      <style>{`
        @media (max-width: 960px) { .hero-grid { grid-template-columns: 1fr !important; padding: 48px 0 64px !important; gap: 40px !important; } .hero-img-col { order: -1; } }
      `}</style>
    </section>
  );
}
