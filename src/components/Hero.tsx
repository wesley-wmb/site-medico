    ...
    <section id="inicio" style={{ background: "radial-gradient(ellipse 70% 60% at 10% 65%, rgba(124,92,69,.08) 0%, transparent 65%), radial-gradient(ellipse 50% 50% at 88% 18%, rgba(124,92,69,.06) 0%, transparent 55%), var(--color-bg)", padding: "0 40px" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "88px 0 96px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }} className="hero-grid">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} style={{ display: "flex", flexDirection: "column" }} className="hero-content-col">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="hero-badge" style={{ display: "inline-flex", alignItems: "center", gap: 8, alignSelf: "flex-start", background: "var(--color-gold-light)", border: ".5px solid rgba(124,92,69,.3)", borderRadius: 9999, padding: "6px 14px", fontSize: 11, fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "var(--color-gold-dark)", marginBottom: 24, whiteSpace: "nowrap" as const, overflow: "hidden" }}>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, delay: 0.1, ease: [0.16, 1, 0.3, 1] }} style={{ position: "relative", animation: "float 6s ease-in-out infinite" }} className="hero-img-col">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.8 }} className="hero-float-card" style={{ position: "absolute", bottom: 32, left: -32, background: "rgba(255,255,255,.85)", backdropFilter: "blur(20px) saturate(160%)", WebkitBackdropFilter: "blur(20px) saturate(160%)", border: ".5px solid rgba(255,255,255,.9)", borderRadius: 20, padding: "14px 18px", boxShadow: "0 12px 40px rgba(0,0,0,.12), 0 2px 8px rgba(0,0,0,.06)", minWidth: 200 }}>
      ...
      <style>{`
        ...
        @media (max-width: 480px) { .hero-badge { font-size: 9px !important; padding: 4px 10px !important; letter-spacing: 0.05em !important; } }
      `}</style>
    </section>
  );
}
