...
const depos = [
  ...
  { initials: "FT", name: "Fábio Tenório", city: "São José dos Pinhais", text: '"Fiz meu tratamento de alopecia areata na Infinity e o problema foi 100% solucionado!!! Continuo fazendo tratamento para alopecia androgenetica e já estou vendo resultados no preenchimento das falhas!!! Recomendo 100%!!!"', tag: "Tricologia", fotos: 1 },
];

const categorias = ["Botox", "Lábios", "Endolaser"];
const casesMeta = [
  ...
  { proc: "Tratamento Corporal Endolaser", time: "30 dias", patient: "Paciente 40 anos · Gordura localizada · Protocolo combinado" },
];
    ...
    <section id="resultados" style={{ background: "var(--color-surface)", padding: "96px 40px" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto" }}>
        ...
        >
          ...
          <div className="cfm-pill" style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "var(--color-gold-light)", border: ".5px solid rgba(124,92,69,.3)", borderRadius: 9999, padding: "6px 14px", whiteSpace: "nowrap" as const, flexShrink: 0 }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" style={{ width: 12, height: 12, color: "var(--color-gold)" }}><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 64 }} className="cases-grid">
          {casesMeta.map((c, i) => (
            <motion.div key={`${activeTab}-${i}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: i * 0.08 }}
              style={{ border: ".5px solid var(--color-border-subtle)", borderRadius: 20, overflow: "hidden", background: "var(--color-surface)", transition: "transform .25s cubic-bezier(.22,1,.36,1), box-shadow .25s" }}
              whileHover={{ y: -4, boxShadow: "0 12px 40px rgba(0,0,0,.1)" }}
            >
              <ImageSlider />
              <div style={{ padding: "14px 16px 16px" }}>
                ...
                <div style={{ fontSize: 10, color: "var(--color-gold-dark)", background: "var(--color-gold-light)", borderRadius: 8, padding: "6px 10px", lineHeight: 1.55, display: "flex", gap: 5, alignItems: "flex-start" }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" style={{ width: 11, height: 11, flexShrink: 0, marginTop: 1 }}><circle cx="12" cy="12" r="10" /><path d="M12 8v4M12 16h.01" /></svg>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }} className="depo-grid">
          {depos.map((d, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
              ...
              whileHover={{ y: -3, boxShadow: "0 4px 16px rgba(0,0,0,.07)", borderColor: "rgba(124,92,69,.2)" }}
            >
